...
func newFixture(t *testing.T) *fixture {
	...
	f.svc.newProvider = func(string) provider.Provider { return f.fake }
	return f
}
...
func (f *fixture) startWithModel(t *testing.T) State {
	...
	if err := f.svc.SetModel(state.ChatID, "fake-model"); err != nil {
...
func TestSetModelPersistsDefault(t *testing.T) {
	...
	raw, ok, err := f.store.GetSetting("provider.ollama.model")
	if err != nil || !ok || raw != `"fake-model"` {
		t.Errorf("default model setting = %q ok=%v err=%v", raw, ok, err)
	}
	if err := f.svc.SetModel(state.ChatID, ""); err == nil {
